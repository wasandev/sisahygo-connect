...
| Reports | Report center, Phase 1 reports, and Shipment Status & Timeline | `/reports`, `/reports/shipments`, `/reports/shipment-status`, `/reports/order-checkings`, `/reports/payments` | `ReportCenter`, `ReportPage`, `ReportQueryService`, `ReportsEndpoint`, `ReportExportController` | Core `/reports/*` Client API endpoints including `/reports/shipment-status` | Working | Focused | Focused | Completed | Requires `report.view`; Excel requires `report.export`; shipment-status includes expandable timeline, cancelled shipments, delivery as in-progress, and delayed/in-progress filters. |
