...
| Reports | Report center and Phase 1 report pages | `/reports`, `/reports/shipments`, `/reports/order-checkings`, `/reports/payments` | `ReportCenter`, `ReportPage`, `ReportQueryService`, `ReportsEndpoint` | Core `/reports/*` Client API endpoints | Working | Focused | Focused | Completed | Requires `report.view`; Excel requires `report.export`. |
