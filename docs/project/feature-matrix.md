...
| Reports | Report center, Phase 1 reports, Shipment Status & Timeline, Receiver & Delivery Area, and Product & Shipment Volume | `/reports`, `/reports/shipments`, `/reports/shipment-status`, `/reports/receivers`, `/reports/products`, `/reports/order-checkings`, `/reports/payments` | `ReportCenter`, `ReportPage`, `ReportQueryService`, `ReportsEndpoint`, `ReportExportController` | Core `/reports/*` Client API endpoints including `/reports/shipment-status`, `/reports/receivers`, and `/reports/products` | Working | Focused | Focused | Completed | Requires `report.view`; Excel requires `report.export`; no new capability. Receiver/product reports reuse shipment visibility, SQL aggregation, HTML pagination, and Excel export sheets. |
...
## Reports Phase 2.1 Addendum

Shipment Status & Timeline is implemented locally. It adds `/reports/shipment-status`, summary cards, filters for delayed and in-progress shipments, expandable timeline rows, and an Excel export with Summary, Shipment Status, and Timeline sheets. Delayed status is calculated by Sisahygo from processing elapsed time and configurable threshold; delivery is in-progress and cancelled shipments remain historical report rows. No new capability was added; access continues to use `report.view` and `report.export`. No commit, push, or deploy has been performed.


## Reports Phase 2.2 and 2.3 Addendum

Receiver & Delivery Area and Product & Shipment Volume are implemented locally. Receiver reports aggregate authorized shipment activity by receiver and structured destination area, with Summary, Receivers, and Delivery Areas export sheets. Product reports aggregate authorized shipment detail lines by Product + Unit, keep distinct shipment counts, use detail-level freight only, and export Summary, Products, and Product Details sheets. Access continues to use `report.view` and `report.export`; no new capabilities, migrations, dependencies, commit, push, or deploy have been performed.
